function Dog() {
  ...
  this.getCurrentPercentage = function() {
    ...
    var target =  usage / cap * 100;
    this.log("dog.getCurrentPercentage(): target =" + target);
    var pattern = /(\d{1,4})/g
    var results = pattern.exec(target);
    this.log("dog.getCurrentPercentage() results = " + results);
  ...
  this.setBadge = function(text, isp) {
    this.log("dog.setBadge() isp " + isp.formRegex + " " + isp.formModifier);
