function Dog() {
  ...
  this.start = function(html) {
    ...
    xhr.onreadystatechange = function() {
      if (xhr.readyState == 4 ) {
        var isp = JSON.parse(xhr.responseText);
        dog.setBadge(html, isp);
      }
    }
  }
  ...
  this.run = function() {
    ...
    if(email != 'undefined' && password != 'undefined') {
      this.scrapeISP(email, password);
    }
  };

  this.scrapeISP = function(email, password) {
    dog.log("dog.scrapeISP()");
    this.log(email);
    this.log(password);
